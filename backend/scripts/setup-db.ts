...
const parseDbUrl = (url: string) => {
  ...
  if (!matches) {
    throw new Error('Invalid DATABASE_URL format. Must be postgresql://user:pass@host:port/db');
  }
  return {
    user: matches[1],
    password: matches[2],
...
const setup = async () => {
  ...
  const defaultDbUrl = databaseUrl.replace(`/${dbConfig.database}`, '/postgres');

  console.log(`Connecting to default database to verify/create "${dbConfig.database}"...`);
  const client = new pg.Client({
    connectionString: defaultDbUrl,
    ssl: defaultDbUrl.includes('neon.tech') ? { rejectUnauthorized: false } : false
  });
  ...
  try {
    ...
    if (res.rowCount === 0) {
      ...
      // CREATE DATABASE cannot run inside a transaction block, so we run it on client directly
  ...
  // Connect to the actual target database to run schema and seeds
  console.log(`Connecting to target database "${dbConfig.database}" to run schema and seed data...`);
  ...
  try {
    ...
    console.log('Applying schema rules...');
    await targetClient.query(sql);
    console.log('Tables, indexes, and triggers successfully established.');

    // 2. Seed default data
    console.log('Inserting seed users, products, customers, follow-ups, and draft challans...');
    ...
    // Seed users
    const userRes = await targetClient.query(`
    ...
    // Seed Customers
    const customerRes = await targetClient.query(`
    ...
    // Seed Products
    const productRes = await targetClient.query(`
      INSERT INTO products (product_name, sku, category, unit_price, current_stock, min_stock_alert, location, created_by) VALUES
      ('Dell Latitude Laptop', 'ELEC001', 'Electronics', 45000.00, 50, 5, 'Row A - Shelf 2', $1),
      ('ThinkPad X1 Carbon', 'ELEC002', 'Electronics', 85000.00, 10, 3, 'Row A - Shelf 3', $1),
      ('Classic Laptop Bag', 'BAG001', 'Accessories', 1200.00, 100, 15, 'Row B - Shelf 1', $1),
      ('Ergonomic Office Chair', 'FURN001', 'Furniture', 8500.00, 15, 2, 'Row C - Shelf 4', $1),
      ('Gel Pen Blue Box', 'OFF001', 'Office Supplies', 120.00, 200, 20, 'Row D - Shelf 1', $1),
      ('HP Laser Jet Printer', 'ELEC003', 'Electronics', 18500.00, 2, 5, 'Row A - Shelf 4', $1),
      ('Type-C USB Hub 6-in-1', 'ACC001', 'Accessories', 1500.00, 0, 10, 'Row B - Shelf 2', $1),
      ('Standing Computer Desk', 'FURN002', 'Furniture', 14500.00, 25, 5, 'Row C - Shelf 1', $1)
      RETURNING id, sku;
    `, [warehouseId]);
    ...
    `, [pLaptop, pThinkpad, pBag, pChair, pPen, pPrinter, pPrinter, warehouseId]);

    // Seed draft challans
    const ch1Res = await targetClient.query(`
    ...
    const ch2Res = await targetClient.query(`
      INSERT INTO challans (challan_number, customer_id, total_quantity, status, created_by) VALUES
    ...
    const ch3Res = await targetClient.query(`
      INSERT INTO challans (challan_number, customer_id, total_quantity, status, created_by) VALUES
    ...
    console.error('Error executing database setup script:', error);
    process.exit(1);
  } finally {
